...
let private tabs (dispatch: Dispatch) (current: LearnPage) =
    Html.nav [
        ...
        prop.custom ("aria-label", "Learn")
        prop.children [
            for text, page in [ "Contents", LearnContents; "Letters", LearnLetters; "Sounds", LearnSounds; "Iliad", LearnIliad; "Myth", LearnMyth ] do
                Html.a [
                    prop.key text
                    prop.href (Router.learnHash page)
...
let private preface (lm: LearnModel) (dispatch: Dispatch) =
    leaf "" [
        ...
        note [ Html.text "You can change this later on the Learn contents page." ]
...
let private contents (lm: LearnModel) (dispatch: Dispatch) =
...
let private lettersPage (lm: LearnModel) (dispatch: Dispatch) =
    ...
    leaf "lx-letters" [
        ...
        note [ Html.text "Sounds follow the restored Attic of the fifth century BC. Playback uses your device’s modern Greek voice, where it has one." ]
    ]
                            ...
                            Html.span [
                                prop.children [
                                    for wi, w in Array.indexed words do
                                        Html.button [
                                            prop.key wi
                                            prop.classes [ "lx-w"; if w.FirstDecl then "d1"; if lm.Gloss = Some(li, wi) then "on" ]
